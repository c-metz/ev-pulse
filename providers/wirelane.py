"""wirelane – Mobilithek DATEX II provider.
...
Differences from eco-movement:
  - Location and operator live at **station** level, not site level.
  - Uses ``locPointLocation`` on the station (sites have no ``locationReference``).
  - Address fields (city, postcode, country) are typically absent; the
    site name often embeds the address, e.g. "Berlin - Yorckstraße 25".
  - Operator is on the station: ``afacAnOrganisation.name``.
  - No ``availableChargingPower`` on charging points.
"""
...
class WirelaneProvider(Provider):
    name = "wirelane"
    slug = "wirelane"
        ...
        for table in pub.get("energyInfrastructureTable", []):
            for site in table.get("energyInfrastructureSite", []):
                ...
                for station in site.get("energyInfrastructureStation", []):
                    ...
                    # Location lives on the *station*, not the site
                    loc = station.get("locationReference", {})
                    point_loc = loc.get("locPointLocation", {})
                    area_loc = loc.get("locAreaLocation", {})
                    coords = (
                        point_loc.get("coordinatesForDisplay", {})
                        or area_loc.get("coordinatesForDisplay", {})
                    )
                    latitude = coords.get("latitude")
                    longitude = coords.get("longitude")

                    loc_ext = (
                        point_loc.get("locLocationExtensionG", {})
                        or area_loc.get("locLocationExtensionG", {})
                    )
                    ...
                    # Operator lives on the station too
                    operator_name = None
                    operator_obj = station.get("operator", {})
