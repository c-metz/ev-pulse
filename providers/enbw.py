"""EnBW – Mobilithek DATEX II provider.

Static data : subscription endpoint (JSON, mTLS).
Dynamic data: subscription endpoint (JSON deltaPull, mTLS).

EnBW uses ``aegiRefillPointStatus`` (not ``aegiElectricChargingPointStatus``)
in its dynamic feed.  Point IDs are EVSE-style (e.g. "DE*EBW*E904644*1").
...
# ── Endpoints ─────────────────────────────────────────────────────────
STATIC_SUBSCRIPTION_ID = "970702332500893696"
STATIC_URL = (
    f"https://mobilithek.info:8443/mobilithek/api/v1.0/subscription"
    f"?subscriptionID={STATIC_SUBSCRIPTION_ID}"
)

DYNAMIC_SUBSCRIPTION_ID = "970702311915421696"
DYNAMIC_URL = (
    f"https://mobilithek.info:8443/mobilithek/api/v1.0/subscription"
    f"?subscriptionID={DYNAMIC_SUBSCRIPTION_ID}"
)

FETCH_TIMEOUT = 300  # 66 MB static payload


class EnbwProvider(Provider):
    name = "EnBW"
    slug = "enbw"
            ...
            for site_st in pub.get("energyInfrastructureSiteStatus", []):
                for station_st in site_st.get("energyInfrastructureStationStatus", []):
                    for rp in station_st.get("refillPointStatus", []):
                        # EnBW uses aegiRefillPointStatus
                        ecp = rp.get("aegiRefillPointStatus", {})
                        if not ecp:
