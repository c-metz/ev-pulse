"""msu – Mobilithek DATEX II provider.
...
Differences from eco-movement:
  - Operator field is a *list*, each element has ``afacOrganisation.organisationName``
    (not ``afacAnOrganisation.name``).
  - Location uses ``locAreaLocation`` (same as eco-movement).
  - Dynamic payload has only ``status`` (no opening/operation status).
"""
    ...
    def _parse_static_points(self, data: dict) -> pd.DataFrame:
        ...
        for table in pub.get("energyInfrastructureTable", []):
            for site in table.get("energyInfrastructureSite", []):
                ...
                # Location: locAreaLocation (like eco-movement)
                ...
                # Operator: a *list* with afacOrganisation.organisationName
                operator_name = None
                operator_obj = site.get("operator")
                if isinstance(operator_obj, list) and operator_obj:
                    org = operator_obj[0].get("afacOrganisation", {})
                    operator_name = get_name(org.get("organisationName"))
                elif isinstance(operator_obj, dict):
                    org = operator_obj.get("afacOrganisation", {})
                    operator_name = get_name(org.get("organisationName"))
