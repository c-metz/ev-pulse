...
for slug in selected_providers:
    ...
    if static is None or static.empty:
        continue
    cfg = PROVIDERS[slug]
    # Normalise to kW — eco uses watts, tesla uses kW natively
    if "station_max_power_w" in static.columns:
        vals = pd.to_numeric(static["station_max_power_w"], errors="coerce").dropna() / 1000
    elif "station_max_power_kw" in static.columns:
        vals = pd.to_numeric(static["station_max_power_kw"], errors="coerce").dropna()
    else:
        continue
    if vals.empty:
        continue
    power_hist_data[slug] = vals

if power_hist_data:
    fig_pw = go.Figure()
    for slug, vals in power_hist_data.items():
        cfg = PROVIDERS[slug]
        fig_pw.add_trace(go.Histogram(
            x=vals,
            name=cfg["label"],
            marker_color=cfg["color"],
            opacity=0.7,
            xbins=dict(start=0, end=min(vals.quantile(0.99) * 1.1, vals.max()), size=20),
            hovertemplate="%{x:.0f} kW — %{y:,} stations<extra>" + cfg["label"] + "</extra>",
        ))
    ...
    fig_pw.update_xaxes(gridcolor="rgba(128,128,128,0.12)")
    fig_pw.update_yaxes(gridcolor="rgba(128,128,128,0.12)")
    st.plotly_chart(fig_pw, use_container_width=True)

    # Summary metrics
    pw_metric_cols = st.columns(len(power_hist_data))
    for col, (slug, vals) in zip(pw_metric_cols, power_hist_data.items()):
        cfg = PROVIDERS[slug]
    ...
    st.info("No station power data available.")
